...
- `Suspended :` Giống như trạng thái `block`, những task trong trạng thái `Suspend` không thể được chọn để vào trạng thái `Running`, nhưng những task bị `Suspend` không có time out. Thay vì, những task chỉ vào hoặc thoát `Suspend` khi có lệnh thông qua `vTaskSuspend()` và `xTaskResume()`.

![alt text](/assets/RTOS/task_state.png)

## 🐨 Task properties
Mỗi task thì được giao 1 priority từ 0 đến (configMAX_PRIORITIES - 1) được define trong FreeRTOSConfig.h.
