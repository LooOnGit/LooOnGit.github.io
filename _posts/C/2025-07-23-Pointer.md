...
## 🧩 Con trỏ const
Trong C, từ khóa const đi với con trỏ có nhiều “ý nghĩa” khác nhau tùy vị trí đặt, nhưng mục tiêu chính là bảo vệ dữ liệu, tránh bug, và làm rõ ý đồ của người viết code.

### 📌 1. Các dạng const với con trỏ
**(1) const int *p:**
- Dữ liệu không được thay đổi qua con trỏ.
- Con trỏ có thể trỏ sang nơi khác.
**Ví dụ:**
```c
const int x = 10;
const int *p = &x;
//*p = 20; // ❌ lỗi, vì dữ liệu const
int y = 30;
p = &y; // ✅ ok, con trỏ đổi chỗ được
```

**(2) int * const p:**
- Con trỏ cố định, không trỏ sang chỗ khác.
- Nhưng dữ liệu tại chỗ đó có thể sửa được.
**Ví dụ:**
```c
int x = 10;
int * const p = &x;
*p = 20; // ✅ ok, sửa dữ liệu
//int y = 30;
//p = &y; // ❌ lỗi, không đổi địa chỉ
```
**(3) const int * const p:**
- Vừa dữ liệu không đổi, vừa con trỏ không đổi.
- Hoàn toàn bất biến.
```c
int x = 10;
int y = 20;
...
*p = 15;   // ❌ Lỗi: không thể thay đổi giá trị mà p trỏ đến
p = &y;    // ❌ Lỗi: không thể thay đổi địa chỉ p đang giữ
```
Chỉ có cách thay đổi giá trị của x trực tiếp, chứ không qua p:
```c
x = 15;   // ✅ Hợp lệ
```
dùng const để tối ưu hóa compiler.
