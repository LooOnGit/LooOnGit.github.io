...
Hình ảnh về hàng đợi rất hay gặp trong đời sống hàng ngày. Ví dụ điển hình nhất là việc xếp hàng:
...
2. **Dequeue()** ➖
   - Xóa phần tử khỏi đầu (front) của Queue
   - Trả về phần tử đã xóa

3. **isEmpty()** 🔍
   - Kiểm tra Queue rỗng
   - Trả về true nếu Queue không có phần tử nào

4. **Front()** 👀
   - Lấy giá trị của phần tử ở đầu (front) của Queue
   - Không làm thay đổi cấu trúc Queue

## 💾 Cài đặt Queue trong C

Để cài đặt Queue, chúng ta sẽ cần sử dụng các biến như sau:

```c
// Khai báo cấu trúc Queue
typedef struct {
    int* queue;        // Mảng một chiều để lưu trữ phần tử
    int arraySize;     // Số lượng phần tử tối đa có thể lưu trữ
    int front;         // Chỉ số phần tử đầu queue
    int rear;          // Chỉ số phần tử cuối queue
} Queue;
```

### Ví dụ minh họa Dequeue
...
> 💡 **Giải thích**: Sau khi thực hiện Dequeue, phần tử 22 được lấy ra khỏi Queue, front di chuyển từ vị trí 0 sang vị trí 1.

# �🔰 Nội dung bổ sung về Queue
